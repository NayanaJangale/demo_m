...
class MngBrancwiseCircular extends StatefulWidget {
  String menuName;

  MngBrancwiseCircular({
    this.menuName,
  });

  @override
  _MngBrancwiseCircularState createState() =>
      _MngBrancwiseCircularState();
}

class _MngBrancwiseCircularState extends State<MngBrancwiseCircular> with SingleTickerProviderStateMixin{
  bool isLoading;
  String loadingText;
    ...
    return CustomProgressHandler(
      ...
      child: DefaultTabController(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            title: CustomAppBar(
              title: AppTranslations.of(context).text("Circulars"),
              subtitle: AppTranslations.of(context).text("key_circular_report"),
            ),
            actions: <Widget>[
              IconButton(
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => MngAddInternalCircularPage(_branches[tabController.index].brcode),
                    ),
                  );
                },
              ),
  ...
  Widget selectPage(String menuName, String sbrcode) {
    switch (menuName) {
      case MenuNameConst.Circulars:
        return MngInternalCircularPage(
          brcode : sbrcode,
        );
        break;
     }
